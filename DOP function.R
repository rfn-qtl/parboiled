#####################################
# DOP analytic pipeline - function version
...
#################################
# fitting a model for each trial and trait
#################################
require(foreach)
require(doParallel)
require(doMC)
...
system.time(
  results.st <- foreach(i = 1:nrow(grid), 
      ...
      # subset the data  
   sample <- droplevels.data.frame(data[data$plantingDate == grid[i,1] & data$variable == grid[i,2] ,])
...
h2g.MET <- vpredict(fitMET, h2 ~ V1 / ( V1 + V2)) # MET level

# predicting the BLUP - main effect
BLUPs.main <- predict(object = fitMET, D = "germplasmName")$pvals
# reliability
rel <- mean(1 - BLUPs.main$std.error^2 / summary(fitMET)$varcomp[2,1])

# predicting the BLUP per environment
BLUPs.MET <- sample[,c(1,2,7)]
colnames(BLUPs.MET)[2] <- "predicted.value"

rice.handbook.table <- round(reshape2::acast(BLUPs.MET, germplasmName ~  DOP, value.var = "predicted.value"))
...
limits <- aes(ymax = data.plot$predicted.value + data.plot$std.error*1.96,
              ymin = data.plot$predicted.value - data.plot$std.error*1.96)
...
p <- ggplot(data = data.plot, aes(x = reorder(germplasmName, -predicted.value), y = predicted.value, color = reorder(germplasmName, -predicted.value))) + 
  ...
  annotate("text", x=length(unique(data.plot$germplasmName))/2, 
           ...
           label= paste("Reliability = ", round(h2g.MET, 2)*100, "%")) +
  labs(x = "", fill = "DOP") +
  labs(color = "") + 
  coord_cartesian(ylim=c(min(data.plot$predicted.value - data.plot$std.error*1.96),
                         max(data.plot$predicted.value + data.plot$std.error*1.96)))
...
data.plot$std.error <- data.plot$std.error*1.96
colnames(data.plot) <- c("Variety", traits[i], "Confidence_interval")
...
out.MET <- rbind(out.MET, data.frame(
  ...
  Number_of_DOP = length(unique(sample$DOP)),
  Reliability = rel,
...
data.MET <- BLUPs.MET
colnames(data.MET)[1:2] <- c("Variety", traits[i]) 
...
ggsave(filename = paste0("output/", study, "_", traits[i], '_stability_adaptability_over_DOP.tiff'),
       plot = q,
       device = 'tiff',
       width = 300,
       height = 400,
...
# GGE Biplot
model <- gge(data.MET, DOP, Variety, traits[i], svp = "symmetrical")
a <- plot(model, type = 1)
b <- plot(model, type = 2)
c <- plot(model, type = 3)
...
q <- ggplot(data = data.MET, aes(y = data.MET[,2], 
                                 x = DOP, 
                                 fill = Variety)) +
...
ggsave(filename = paste0("output/", study, "_", traits[i], '_performances_by_DOP.tiff'),
       plot = q,
       device = 'tiff',
       width = 400,
       height = 250,
...
#### Final report
# saving the output file for single-trials analysis
write.csv(out.MET, paste0("output/", study, "_", "KPI_report.csv"))
